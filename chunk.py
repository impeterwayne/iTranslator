...
def llm_translate_chunk(masked_chunk: str, target_language: str, app_name: str, llm, provider: str) -> str:
    prompt = f"""You are a professional application translator. Translate this Android strings XML snippet to ISO language code: {target_language} for the "{app_name}" app.
...
Translated XML:"""
    ...
    # Trim to first '<'
    ix = text.find('<')
    return text[ix:] if ix >= 0 else text
# -------------------------------------------------------------------
